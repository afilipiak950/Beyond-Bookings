...
import { insertPricingCalculationSchema, insertFeedbackSchema, insertOcrAnalysisSchema, createUserSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // User Management Routes - Only for test@example.com
  app.get('/api/users', requireAuth, async (req: any, res) => {
    try {
      const currentUser = await storage.getUser(req.user.id);
      if (!currentUser || !await storage.canCreateUsers(currentUser.email)) {
