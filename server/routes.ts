...
Only return hotel data if you can verify this is a real, existing hotel. Do not make up information.`;
  ...
  app.post('/api/export/comprehensive-pdf', requireAuth, async (req: any, res) => {
    try {
      ...
      // Helper function for adding headers with styling
      const addHeader = (text: string, size = 18, color = '#1a365d') => {
        doc.fontSize(size)
           .fillColor(color)
           .font('Helvetica-Bold')
      ...
      const addSubheader = (text: string, size = 14, color = '#2d3748') => {
        doc.fontSize(size)
           .fillColor(color)
           .font('Helvetica-Bold')
           .text(text, { align: 'left' })
      ...
      const addText = (text: string, size = 10, color = '#4a5568') => {
        doc.fontSize(size)
           .fillColor(color)
           .font('Helvetica')
           .text(text, { align: 'left' })
      ...
      doc.fontSize(28)
         .fillColor('#1a365d')
         .font('Helvetica-Bold')
         .text(config?.title || 'Hotel Pricing Analysis Report', { align: 'center' })
         .moveDown(2);

      doc.fontSize(16)
         .fillColor('#3182ce')
         .font('Helvetica')
         .text('Comprehensive Financial Analysis & Market Intelligence', { align: 'center' })
         .moveDown(3);

      // Company branding section
      doc.fontSize(14)
         .fillColor('#2d3748')
         .text(`Generated by: ${config?.companyName || 'bebo convert'}`, { align: 'center' })
         .moveDown(0.5);

      if (config?.authorName) {
        doc.text(`Report Author: ${config.authorName}`, { align: 'center' })
           .moveDown(0.5);
      }

      doc.text(`Report Date: ${new Date(config?.reportDate || Date.now()).toLocaleDateString('de-DE')}`, { align: 'center' })
         .moveDown(0.5);

      doc.text(`Analysis Period: ${calculations.length} Hotel Calculations`, { align: 'center' })
         .moveDown(4);

      // Key metrics box
      const metricsBoxY = doc.y;
      doc.rect(75, metricsBoxY, doc.page.width - 150, 120)
         .fillAndStroke('#f7fafc', '#e2e8f0');

      doc.fillColor('#1a365d')
         .fontSize(12)
         .font('Helvetica-Bold')
         .text('Executive Summary Metrics', 90, metricsBoxY + 15);

      const metricsStartY = metricsBoxY + 40;
      doc.fontSize(10).fillColor('#2d3748').font('Helvetica');
      
      doc.text(`Total Hotels Analyzed: ${analytics?.totalHotels || calculations.length}`, 90, metricsStartY);
      doc.text(`Average Profit Margin: €${(analytics?.averageProfit || 0).toFixed(2)}`, 90, metricsStartY + 15);
      doc.text(`Total Revenue Potential: €${(analytics?.totalRevenue || 0).toFixed(2)}`, 90, metricsStartY + 30);
      doc.text(`Average Star Rating: ${(analytics?.averageStars || 0).toFixed(1)} Stars`, 90, metricsStartY + 45);
      doc.text(`Average Occupancy Rate: ${(analytics?.averageOccupancy || 0).toFixed(1)}%`, 90, metricsStartY + 60);
      ...
      if (config?.includeExecutiveSummary) {
        addHeader('Executive Summary', 20, '#1a365d');
        
        addText('This comprehensive report analyzes hotel pricing strategies and financial performance across multiple properties, providing actionable insights for revenue optimization and market positioning.');
        
        addSubheader('Market Overview');
        addText(`This analysis covers ${calculations.length} pricing calculations across ${analytics?.totalHotels || 0} unique hotel properties.`);
        addText(`The portfolio shows an average profit margin of €${(analytics?.averageProfit || 0).toFixed(2)} per calculation with total revenue potential of €${(analytics?.totalRevenue || 0).toFixed(2)}.`);
        
        if (analytics?.topCities && analytics.topCities.length > 0) {
          addSubheader('Geographic Distribution');
          analytics.topCities.forEach((city: any, index: number) => {
            addText(`${index + 1}. ${city.city}: ${city.count} properties`);
          });
        }

        addSubheader('Performance Highlights');
        addText(`• Average occupancy rate of ${(analytics?.averageOccupancy || 0).toFixed(1)}% across all properties`);
        addText(`• Properties maintain an average ${(analytics?.averageStars || 0).toFixed(1)}-star rating`);
        addText(`• Revenue optimization opportunities identified across multiple price segments`);
        
        doc.addPage();
      }

      // PAGE 3: DETAILED CALCULATIONS
      if (config?.includeDetailedCalculations) {
        addHeader('Detailed Financial Calculations', 18, '#1a365d');
        
        addText('The following section provides comprehensive financial analysis for each hotel property, including pricing breakdowns, profit margins, and revenue projections.');
        
        // Table headers
        const headers = ['Hotel', 'Stars', 'Price (€)', 'Profit (€)', 'Occupancy', 'Revenue (€)'];
        addTableRow(headers, true);
        
        // Add each calculation
        calculations.slice(0, 20).forEach((calc: any) => { // Limit to first 20 for PDF space
          ...
          const revenue = price * rooms * (occupancy / 100);
          
          const row = [
            (calc.hotelName || 'Unknown').substring(0, 20),
            (calc.stars || 0).toString(),
            price.toFixed(0),
            profit.toFixed(0),
            `${occupancy.toFixed(0)}%`,
            revenue.toFixed(0)
        ...
        if (calculations.length > 20) {
          addText(`... and ${calculations.length - 20} additional calculations (see appendix for complete data)`);
        }
      ...
      if (config?.includeMarketAnalysis) {
        addHeader('Market Analysis & Positioning', 18, '#1a365d');
        
        addSubheader('Price Range Analysis');
        const prices = calculations.map((calc: any) => parseFloat(calc.totalPrice?.toString() || "0"));
        const minPrice = Math.min(...prices);
        const maxPrice = Math.max(...prices);
        const avgPrice = prices.reduce((sum, price) => sum + price, 0) / prices.length;
        
        addText(`Price Range: €${minPrice.toFixed(2)} - €${maxPrice.toFixed(2)}`);
        addText(`Average Price: €${avgPrice.toFixed(2)}`);
        addText(`Standard Deviation: €${Math.sqrt(prices.reduce((sum, price) => sum + Math.pow(price - avgPrice, 2), 0) / prices.length).toFixed(2)}`);
        
        addSubheader('Star Rating Distribution');
        const starCounts = calculations.reduce((acc: any, calc: any) => {
          const stars = calc.stars || 0;
          acc[stars] = (acc[stars] || 0) + 1;
          return acc;
        }, {});
        
        Object.entries(starCounts).forEach(([stars, count]) => {
          addText(`${stars} Stars: ${count} properties (${((count as number / calculations.length) * 100).toFixed(1)}%)`);
        });
        
        addSubheader('Profitability Analysis');
        ...
        const highProfitCount = profits.filter(p => p > 30).length;
        const mediumProfitCount = profits.filter(p => p >= 20 && p <= 30).length;
        const lowProfitCount = profits.filter(p => p < 20).length;
        
        addText(`Average Profit Margin: €${avgProfitMargin.toFixed(2)}`);
        addText(`High Profit Properties (>€30): ${highProfitCount} (${((highProfitCount / calculations.length) * 100).toFixed(1)}%)`);
        addText(`Medium Profit Properties (€20-30): ${mediumProfitCount} (${((mediumProfitCount / calculations.length) * 100).toFixed(1)}%)`);
        addText(`Low Profit Properties (<€20): ${lowProfitCount} (${((lowProfitCount / calculations.length) * 100).toFixed(1)}%)`);
      ...
      if (config?.includeRecommendations) {
        addHeader('AI-Powered Recommendations', 18, '#1a365d');
        
        addSubheader('Revenue Optimization Opportunities');
        addText('• Consider dynamic pricing strategies for properties with high occupancy rates');
        addText('• Implement seasonal pricing adjustments based on market demand patterns');
        addText('• Focus on premium service offerings for 4+ star properties to justify higher margins');
        addText('• Review operational costs for properties with profit margins below 20%');
        
        addSubheader('Market Positioning Strategies');
        addText('• Leverage competitive pricing for properties in high-demand locations');
        addText('• Develop package deals for properties with lower individual profit margins');
        addText('• Consider investment in amenities for properties below market average star ratings');
        
        addSubheader('Financial Performance Improvements');
        addText('• Optimize VAT strategies across different property types and locations');
        addText('• Implement cost reduction measures for properties with high operational expenses');
        addText('• Explore bulk purchasing opportunities for multi-property portfolios');
        
        doc.addPage();
      }

      // FINAL PAGE: FILTER SUMMARY & METADATA
      addHeader('Report Metadata & Filters Applied', 16, '#1a365d');
      
      addSubheader('Applied Filters');
      if (filters?.cities && filters.cities.length > 0) {
        addText(`Cities: ${filters.cities.join(', ')}`);
      }
      if (filters?.starRatings && filters.starRatings.length > 0) {
        addText(`Star Ratings: ${filters.starRatings.join(', ')} stars`);
      }
      if (filters?.priceRange && filters.priceRange.enabled) {
        addText(`Price Range: €${filters.priceRange.min} - €${filters.priceRange.max}`);
      }
      if (filters?.dateRange && filters.dateRange.enabled) {
        addText(`Date Range: ${filters.dateRange.startDate} to ${filters.dateRange.endDate}`);
      }
      
      addSubheader('Report Generation Details');
      addText(`Total Calculations Analyzed: ${calculations.length}`);
      addText(`Report Generated: ${new Date().toLocaleString('de-DE')}`);
      addText(`Generated by: bebo convert Analytics Platform`);
      addText(`Platform Version: 2.0.0 (Hotel Intelligence Suite)`);
      
      // Footer on last page
      doc.fontSize(8)
         .fillColor('#718096')
         .text('This report was generated by bebo convert - Professional Hotel Analytics Platform', 50, doc.page.height - 80, {
           align: 'center',
